...
3. **Session**：只对 OpenCode Go 渠道生效。把 Codex `Session-Id` 等映射成 `x-opencode-session`，并改写 `xhigh` / `json_schema` / `namespace` tools。其它模型不会加这个头。
4. **本机累计 Token**：成功请求计入输入 / 输出 / 缓存读取 / 缓存写入，插件页和配额页都能看合计（到「亿」）和缓存命中率。数据写在 CPA 目录的 `opencode-session-tokens.json`，不是官方账单。
